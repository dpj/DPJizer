...
class IDEATest {
	...
	int []<R5> DK;            // Decryption subkey (userkey derived).

	/*
<region R1,R2,R3 | R1#R2, R2#R3> void encrypt(int[]<R3> Z, DPJArrayByte<R1> input, DPJArrayByte<R2> output) reads R3, R1:* writes R2:*
{
  if(input.length < 256)
    run1(0,input.length,input, output,Z);
  else {
    int len = input.length;
    int lenby2 = len/2;
    DPJPartitionByte<R1> inputhalves = new DPJPartitionByte<R1>(input, lenby2);
    DPJPartitionByte<R2> outputhalves = new DPJPartitionByte<R2>(output, lenby2);
    cobegin {
      encrypt(Z, inputhalves.get(0), outputhalves.get(0));
//      encrypt(Z, inputhalves.get(1), outputhalves.get(1));
    }
  }
}*/
	...
	void Do()
	{
		...
		System.out.println("Length.." + plain1.length + ", nthreads = " + JGFCryptBench.nthreads);

		/*encrypt(Z, plain1,crypt1); */

		// Encrypt plain1.
		tslice = plain1.length / 8;
		ttslice = (tslice + JGFCryptBench.nthreads-1) / JGFCryptBench.nthreads;
		slice = ttslice*8;
		/*foreach (int i in 0, JGFCryptBench.nthreads)
  {
    run1(0,slice,plain1.subarray(i*slice,slice),crypt1.subarray(i*slice,slice),Z);
  }*/
		...
		/*encrypt(Z, crypt1, plain2);*/

		// Decrypt crypt1
		tslice = crypt1.length / 8;
		ttslice = (tslice + JGFCryptBench.nthreads-1) / JGFCryptBench.nthreads;
		slice = ttslice*8;
		/*  foreach (int i in 0, JGFCryptBench.nthreads)
  {
    run1(0,slice,crypt1.subarray(i*slice,slice),plain2.subarray(i*slice,slice),DK);
  }
		 */
	...
	void buildTestData()
	{
		...
		Z = new int [52]<R4>;         // Encryption subkey (user key derived).
		DK = new int [52]<R5>;        // Decryption subkey (user key derived).
	...
	public <region R1, R2, R3 | R1:* # R2:*, R2:* # R3:*, R1:* # R3:*> void run1 (int ilow, int iupper, DPJArrayByte<R1> text1, DPJArrayByte<R2> text2, int []<R3> key) 
	//	reads R3, R1  writes R2 
	{
		...
		for (int i =ilow ; i <iupper ; i +=8)
		{
			...
			x1 = text1.get(i1++) & 0xff;          // Build 16-bit x1 from 2 bytes,
			x1 |= (text1.get(i1++) & 0xff) << 8;  // assuming low-order byte first.
			x2 = text1.get(i1++) & 0xff;
			x2 |= (text1.get(i1++) & 0xff) << 8;
			x3 = text1.get(i1++) & 0xff;
			x3 |= (text1.get(i1++) & 0xff) << 8;
			x4 = text1.get(i1++) & 0xff;
			x4 |= (text1.get(i1++) & 0xff) << 8;
			...
			text2.put(i2++, (byte) x1);
			text2.put(i2++, (byte) (x1 >>> 8));
			text2.put(i2++, (byte) x3);                // x3 and x2 are switched
			text2.put(i2++, (byte) (x3 >>> 8));        // only in name.
			text2.put(i2++, (byte) x2);
			text2.put(i2++, (byte) (x2 >>> 8));
			text2.put(i2++, (byte) x4);
			text2.put(i2++, (byte) (x4 >>> 8));
