...
public class AppDemo extends Universal {
	...
	commutative void sumReduction(final int index, double localAvgExpectedReturnRateMC,	double localAvgVolatilityMC) 
	 ...
	 */
	public ToTask<[i]>[]<taskR:[i]>#i get_tasks() 
	//	reads taskR 
	{
		return(this.tasks);
	}
	 ...
	 */
	public void set_tasks(ToTask<[i]>[]<taskR:[i]>#i tasks) 
	//	writes taskR 
	{
		this.tasks = tasks;
	}
	 ...
	 */
	public ToResult<[i]>[]<resultR:[i]>#i get_results() 
	//	reads Root,resultR 
	{
		return(this.results);
	}
	 ...
	 */
	public void set_results(ToResult<[i]>[]<resultR:[i]>#i results) 
	//	writes resultR 
	{
