...
public class Quadtree extends Harness {
	...
	makeTree(DPJIndexedSet<Body<R1>,R2> S, Box box, int level)
	//	writes R2:*
	{
		// Nothing to add
		if (S.size() == 0) return null;
		// Only one thing
		if (S.size() == 1) return S.iterator().get(0).<region R2>copy();
		// More than one: Make a new inner node and fill it
		// recursively
		DPJIndexedSet<Body<R1>,R2:[i]>[]<Local:[i]>#i quadrants = 
			(DPJIndexedSet<Body<R1>,R2:[i]>[]<Local:[i]>#i) new DPJIndexedSet[4];
		foreach (int i in 0, 4)
		quadrants[i] = new DPJIndexedSet<Body<R1>,R2:[i]>(S.size());
		foreach (Body<R1> b in S.iterator()) {
		...
		foreach (int i in 0, 4) {
			node.children[i] = 
				this.<region R1, R2:[i]>makeTree(quadrants[i], box.makeQuadrant(i), level+1);
		}
		return node;
	}
	...
	DPJIndexedSet<Body> bodies = new DPJIndexedSet<Body>(size);
	Node root;
	@Override
	...
	@Override
	public void runTest() {
		Set<Body> myBodies = new HashSet<Body>();
		this.<region Root,Root>checkTree(root, myBodies);
		foreach (Body b in bodies.iterator())
	...
	//	reads Root:* 
	{
		if (subroot == null) return;
			...
			foreach (int i in 0, children.length) {
				Node<R1:[i]> child = children[i];
				this.<region R1:[i],R2>checkTree(child, myBodies);
			}
		}
	}
	...
	@Override
	public void runWork() {
		root = this.<region Root,Root>makeTree(bodies, new Box(0, Body.MAX_COORD, 
				0, Body.MAX_COORD), 0);
	}
